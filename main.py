...
translations = {
    "PL": {
        ...
        "footer": "Autorzy: Artur Czarnecki, Dawid Balikowski, Piotr Graczyk, Sebastian Maciak",
    ...
    "EN": {
        ...
        "footer": "Authors: Artur Czarnecki, Dawid Balikowski, Piotr Graczyk, Sebastian Maciak",
...
def show_main_view():
    ...
    save_button_text = translations[current_language].get("save_plot", "Save Plot")
    save_button = ttk.Button(toolbar_frame, text=save_button_text, command=save_plot_to_png)
...
def display_results(results):
    ...
    if high_similarity_results:
        ...
        # Pokazujemy dwa najbliższe wyniki, jeśli nie ma wyników o wysokim podobieństwie
        closest_results = results[:2]
        for translated_word, (score, lang) in closest_results:
            result_label = tk.Label(slide_frame, text=f"{translations[current_language]['translation']}: {translated_word} ({translations[current_language]['language']}: {lang}), {translations[current_language]['similarity']}: {score:.2f}%")
            result_label.pack()

    # Wyświetlanie dwóch najmniej podobnych wyników
    if len(results) > 2:
        # Posortuj wyniki według rosnącego podobieństwa (od najmniejszego do największego)
        lowest_similarity_results = sorted(results, key=lambda x: x[1][0])[:2]
        for translated_word, (score, lang) in lowest_similarity_results:
            result_label = tk.Label(slide_frame, text=f"{translations[current_language]['translation']}: {translated_word} ({translations[current_language]['language']}: {lang}), {translations[current_language]['similarity']}: {score:.2f}%", font=("Arial", 10, "bold"))  # Pogrubienie
            result_label.pack()
